...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        inp_hidden=torch.cat((last_hidden[0],last_hidden[1]),dim=1)
        init_decoder_hidden=self.h_projection(inp_hidden)

        inp_cell=torch.cat((last_cell[0],last_cell[1]),1)
        init_decoder_cell =self.c_projection(inp_cell)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        tmp_dec_hidden=torch.unsqueeze(dec_hidden,dim=2)
        e_t=torch.squeeze(torch.bmm(enc_hiddens_proj,tmp_dec_hidden),2)
